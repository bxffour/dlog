...
import (
	...
	api "github.com/bxffour/dlog/api/v1"
	"github.com/bxffour/dlog/internal/auth"
	"github.com/bxffour/dlog/internal/config"
...
func setupTest(t *testing.T, fn func(*Config)) (rootClient, nobodyClient api.LogClient, cfg *Config, teardown func()) {
	...
	if *debug {
		...
		require.NoError(t, err)
		t.Logf("traces log file name: %s", tracesLogFile.Name())
...
func testConsumePastBoundary(t *testing.T, client, _ api.LogClient, config *Config) {
	...
	if got != want {
		t.Fatalf("got err: %v, want: %v", got, err)
	}
}
...
func testUnauthorized(t *testing.T, _, client api.LogClient, config *Config) {
	...
	require.Equal(t, wantCode, gotCode, fmt.Sprintf("got code: %d, want code: %d\n", gotCode, wantCode))
